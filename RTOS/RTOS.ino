...
void sendTelemetry(const char* jsonStr) {
    ...
    tb["method"] = method;
    tb["uid"] = uid;
    tb["card_count"] = doc["card_count"] | state.cardCount;
    ...
    if (strcmp(event, "ACCESS_GRANTED") == 0) {
        ...
        pushLog("✅ Mở cửa | " + String(method) + " | " + String(uid));
    }
        ...
        pushLog("❌ Từ chối | " + String(method) + " | " + String(uid));
    }
    else if (strcmp(event, "CARD_ADDED") == 0) {
        tb["status"] = "CARD_ADDED";
        state.cardCount = doc["card_count"];

        pushLog("➕ Thêm thẻ | " + String(uid));
    }
    else if (strcmp(event, "CARD_DELETED") == 0) {
        tb["status"] = "CARD_DELETED";
        state.cardCount = doc["card_count"];

        pushLog("➖ Xóa thẻ | " + String(uid));
    }
    else if (strcmp(event, "PASSWORD_CHANGED") == 0) {
        tb["status"] = "PASSWORD_CHANGED";

        pushLog("🔑 Đổi mật khẩu");
    }
    ...
    Serial.println("Gửi: " + String(payload));
